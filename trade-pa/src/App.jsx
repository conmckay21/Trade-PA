...
function Settings({ brand, setBrand, companyId, companyName, userRole, members, user, planTier, userLimit, openAssistantSetup, openFeedback, assistantName, assistantWakeWords, userCommandsCount, usageData, usageCaps }) {
  ...
  // Open the upgrade/plan-picker page. Web: same-tab navigation.
  // Native (iOS + Android): Capacitor Browser plugin opens external system
  // browser, so all plan/pricing UI stays off-app (Apple Guideline 3.1.3(b)
  // safe for iOS, equally clean for Android).
  const openUpgradePage = async () => {
    const url = "https://www.tradespa.co.uk/upgrade.html";
    ...
    if (isNative && window.Capacitor?.Plugins?.Browser) {
      try {
        ...
        // fall through to default open below
      }
    }
    if (isNative) {
...
function AppInner() {
  ...
  const view = viewRaw;
  // settings_subscription_handler
  // When the app boots with ?settings=subscription in the URL (e.g. user
  // returning from Stripe Checkout or Customer Portal), auto-route them to
  ...
  return (
    <div style={S.app}>
      ...
      <div style={(isTablet || isDesktopBrowser) ? { display: "flex", alignItems: "flex-start", width: "100%" } : {}}>
        {isTablet && (() => {
          const TABLET_ICONS = {
            ...
            "Suppliers": "M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z",
            ...
            "Calls": "M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z",
      ...
      <main style={{ ...S.main, paddingTop: view === "AI Assistant" || view === "Reminders" || view === "Notifications" ? 16 : 24, paddingBottom: (isTablet || isDesktopBrowser) ? undefined : "60px", ...((isTablet || isDesktopBrowser) ? { flex: 1, maxWidth: "none", padding: "24px 32px", boxSizing: "border-box" } : {}) }}>
        ...
        {view === "Materials" && <Materials materials={materials} setMaterials={setMaterials} jobs={jobs} user={user} companyId={companyId} setContextHint={setContextHint} setView={setView} />}
        ...
        {view === "Reminders" && <Reminders reminders={reminders} onAdd={add} onDismiss={dismiss} onRemove={remove} dueNow={dueNow} onClearDue={() => setDueNow([])} user={user} brand={brand} db={db} />}
        {view === "Notifications" && (
          <Notifications
