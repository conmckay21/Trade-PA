...
export function Schedule({ jobs, setJobs, customers, setContextHint, user }) {
  ...
  const openAdd = (day) => {
    setAddJobDate(day);
    setForm({ customer: "", address: "", type: "", time: "09:00", value: "", status: "confirmed", notes: "", linkedJobCardId: "" });
    setShowAddJob(true);
  };
          ...
          <div onClick={e => e.stopPropagation()} style={{ ...S.card, maxWidth: 480, width: "100%", marginBottom: 16, borderRadius: 14, padding: 0, overflow: "hidden" }}>
            ...
            <div style={{ padding: "12px 16px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button style={S.btn("primary")} onClick={() => {
                ...
                setForm({ customer: selectedJob.customer, address: selectedJob.address || "", type: selectedJob.type, time: selectedJob.dateObj ? new Date(selectedJob.dateObj).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }) : "09:00", value: selectedJob.value || "", status: selectedJob.status, notes: selectedJob.notes || "", linkedJobCardId: selectedJob.source_type === "job_card" ? (selectedJob.source_ref || selectedJob.id || "") : "" });
              }}>Edit</button>
