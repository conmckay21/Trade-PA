...
export function ReportsTab({ invoices, jobs, materials, customers, enquiries, subbiePayments, brand, user, setContextHint }) {
  ...
  const totalRevenue = paidInvoices.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0);
  const totalOutstanding = outstandingInvoices.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0);

  const vatInvoices = paidInvoices.filter(i => (i.vat_enabled || i.vatEnabled) && !(i.vat_zero_rated || i.vatZeroRated));
  const outputVat = vatInvoices.reduce((s, i) => {
    const gross = parseFloat(i.gross_amount || i.grossAmount || i.amount || 0);
  ...
  const totalMaterialCost = periodMaterials.reduce((s, m) => s + parseFloat(m.unit_price || m.unitPrice || 0) * parseFloat(m.qty || 1), 0);
  const inputVat = periodMaterials.filter(m => m.vatEnabled).reduce((s, m) => {
    const net = parseFloat(m.unit_price || m.unitPrice || 0) * parseFloat(m.qty || 1);
  ...
  const cisGross = cisInvoices.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0);
  const cisDeductions = cisInvoices.reduce((s, i) => s + parseFloat(i.cis_deduction || i.cisDeduction || 0), 0);
  const cisNet = cisGross - cisDeductions;

  // Job profitability
  const jobProfitData = (jobs || [])
    .filter(j => j.status === "completed" && inRange(j.date_obj || j.dateObj || j.date))
    .map(j => {
      const jobInvoices = allInvoices.filter(i => i.customer?.toLowerCase() === j.customer?.toLowerCase() && i.status === "paid");
      const revenue = jobInvoices.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0);
      const jobMaterials = (materials || []).filter(m => m.job?.toLowerCase() === j.customer?.toLowerCase() || m.job === j.title);
      const costs = jobMaterials.reduce((s, m) => s + parseFloat(m.unit_price || m.unitPrice || 0) * parseFloat(m.qty || 1), 0);
  ...
  const customerData = (customers || []).map(c => {
    ...
    const totalSpend = paid.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0);
    const lastJob = (jobs || []).filter(j => j.customer?.toLowerCase() === c.name?.toLowerCase()).sort((a, b) => new Date(b.date_obj || b.dateObj || b.date) - new Date(a.date_obj || a.dateObj || a.date))[0];
    return { name: c.name, totalSpend, invoiceCount: paid.length, outstanding: outstanding.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0), lastJobDate: lastJob?.date_obj || lastJob?.dateObj || lastJob?.date };
  ...
  periodMaterials.forEach(m => {
    ...
    supplierData[s].total += parseFloat(m.unit_price || m.unitPrice || 0) * parseFloat(m.qty || 1);
...
<div class="footer">${businessName} · Generated by Trade PA · ${new Date().toLocaleDateString("en-GB")}</div>
  ...
  const generateReportHTML = () => {
    ...
    if (activeReport === "outstanding") return `
      <table>
        ...
        <tbody>${agedDebtors.map(i => tableRow([i.id, i.customer, fmtH(parseFloat(i.gross_amount || i.grossAmount || i.amount || 0)), `${i.daysOld} days`])).join("")}</tbody>
      </table>`;
    ...
    if (activeReport === "materials") return `
      ...
      <table>
        ...
        <tbody>${periodMaterials.map(m => tableRow([m.item, m.supplier || "—", m.qty, fmtH(m.unit_price || m.unitPrice), fmtH((m.unit_price || m.unitPrice||0)*(m.qty||1))])).join("")}</tbody>
      </table>`;
    ...
    if (activeReport === "cis") return `
      ...
      <table>
        ...
        <tbody>${cisInvoices.map(i => tableRow([i.id, i.customer, fmtH(parseFloat(i.gross_amount || i.grossAmount||i.amount||0)), fmtH(parseFloat(i.cis_deduction || i.cisDeduction||0)), fmtH(parseFloat(i.cis_net_payable || i.cisNetPayable||0))])).join("")}</tbody>
      </table>
    ...
    if (activeReport === "jobs") return `
      <table>
        ...
        <tbody>${periodJobs.map(j => tableRow([j.title||j.type||"Job", j.customer, j.type||"—", j.date ? new Date(j.date_obj || j.dateObj||j.date).toLocaleDateString("en-GB") : "TBC", j.status])).join("")}</tbody>
      </table>`;
    ...
    if (activeReport === "quoteconv") {
      ...
      return `
        <div class="stat-grid">
          ${statBox("Total Quotes", allQuotes.length, fmtH(allQuotes.reduce((s,q)=>s+parseFloat(q.gross_amount || q.grossAmount||q.amount||0),0)))}
          ${statBox("Converted", accepted.length, fmtH(accepted.reduce((s,q)=>s+parseFloat(q.gross_amount || q.grossAmount||q.amount||0),0)))}
          ${statBox("Declined", declined.length, fmtH(declined.reduce((s,q)=>s+parseFloat(q.gross_amount || q.grossAmount||q.amount||0),0)))}
          ${statBox("Conversion Rate", rate + "%", `${pending.length} pending`)}
        </div>
        <table>
          ${tableHead(["Quote", "Customer", "Value", "Sent", "Outcome"])}
          <tbody>${allQuotes.sort((a,b)=>new Date(b.created_at||b.date)-new Date(a.created_at||a.date)).map(q => tableRow([q.id, q.customer, fmtH(parseFloat(q.gross_amount || q.grossAmount||q.amount||0)), q.date ? new Date(q.created_at||q.date).toLocaleDateString("en-GB") : "—", q.status==="accepted" ? "Converted" : q.status==="declined" ? "Declined" : "Pending"])).join("")}</tbody>
        </table>`;
    }
  ...
  const generateReportCSV = () => {
    ...
    if (activeReport === "pl") {
      ...
      paidInvoices.forEach(i => csv += row([i.id, i.customer, i.paidDate || i.created_at || i.date, num(i.gross_amount || i.grossAmount || i.amount)]));
      ...
      [["0-30 days", 0, 30], ["31-60 days", 31, 60], ["61-90 days", 61, 90], ["90+ days", 91, 9999]].forEach(([label, min, max]) => {
        ...
        bucket.forEach(i => csv += row([label, i.id, i.customer, num(i.gross_amount || i.grossAmount || i.amount), i.daysOld, i.created_at || i.date]));
      });
      ...
      periodMaterials.forEach(m => {
        const total = parseFloat(m.unit_price || m.unitPrice || 0) * parseFloat(m.qty || 1);
        csv += row([m.item, m.supplier || "", m.qty || 1, num(m.unit_price || m.unitPrice), num(total), m.job || "", m.receiptDate || m.created_at || ""]);
      });
      ...
      cisInvoices.forEach(i => {
        const gross = parseFloat(i.gross_amount || i.grossAmount || i.amount || 0);
        const ded = parseFloat(i.cis_deduction || i.cisDeduction || 0);
      ...
      periodJobs.forEach(j => csv += row([j.customer, j.type || j.title || "", j.status || "", num(j.value), j.date_obj || j.dateObj || j.date || ""]));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingBottom: 80 }}>
      ...
      {activeReport === "pl" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
            ...
            <TableRow cells={["Overdue invoices", fmt(overdueInvoices.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0))]} />
          </div>
          <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
            ...
            {(() => {
              ...
              paidInvoices.forEach(i => {
                ...
                byMonth[key] = (byMonth[key] || 0) + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0);
              });
      ...
      {activeReport === "outstanding" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 10 }}>
            ...
            <StatBox label="Overdue" value={fmt(overdueInvoices.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0))} sub={`${overdueInvoices.length} overdue`} color={C.red} />
          </div>
          {[["0–30 days", 0, 30], ["31–60 days", 31, 60], ["61–90 days", 61, 90], ["90+ days", 91, 9999]].map(([label, min, max]) => {
            const bucket = agedDebtors.filter(i => i.daysOld >= min && i.daysOld <= max);
            if (bucket.length === 0) return null;
            const total = bucket.reduce((s, i) => s + parseFloat(i.gross_amount || i.grossAmount || i.amount || 0), 0);
            return (
              <div key={label} style={{ background: C.surface, borderRadius: 10, border: `1px solid ${min > 30 ? C.red + "44" : C.border}`, overflow: "hidden" }}>
                ...
                {bucket.map(i => <TableRow key={i.id} cells={[i.id, i.customer, fmt(parseFloat(i.gross_amount || i.grossAmount || i.amount || 0)), `${i.daysOld}d`]} />)}
              </div>
            );
          })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
            ...
            {periodMaterials.map((m, i) => (
              <TableRow key={i} cells={[m.item, m.supplier || "—", m.qty, fmt(m.unit_price || m.unitPrice), fmt((m.unit_price || m.unitPrice || 0) * (m.qty || 1))]} />
            ))}
          ...
          <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
            ...
            {cisInvoices.map(i => (
              <TableRow key={i.id} cells={[
                i.id, i.customer,
                fmt(parseFloat(i.gross_amount || i.grossAmount || i.amount || 0)),
                fmt(parseFloat(i.cis_deduction || i.cisDeduction || 0)),
                fmt(parseFloat(i.cis_net_payable || i.cisNetPayable || 0))
              ]} />
            ))}
      ...
      {activeReport === "jobs" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
            ...
            {periodJobs.sort((a, b) => new Date(b.date_obj || b.dateObj || b.date) - new Date(a.date_obj || a.dateObj || a.date)).map((j, i) => {
              ...
              <TableRow key={i} cells={[
                ...
                j.customer,
                j.type || "—",
                j.date ? new Date(j.date_obj || j.dateObj || j.date).toLocaleDateString("en-GB", { day: "numeric", month: "short" }) : "TBC",
      ...
      {activeReport === "quoteconv" && (() => {
        ...
        const totalQuoteValue = allQuotes.reduce((s, q) => s + parseFloat(q.gross_amount || q.grossAmount || q.amount || 0), 0);
        const wonValue = acceptedQuotes.reduce((s, q) => s + parseFloat(q.gross_amount || q.grossAmount || q.amount || 0), 0);
        const lostValue = declinedQuotes.reduce((s, q) => s + parseFloat(q.gross_amount || q.grossAmount || q.amount || 0), 0);
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
              ...
              {allQuotes.sort((a, b) => new Date(b.created_at || b.date) - new Date(a.created_at || a.date)).map((q, i) => (
                <TableRow key={i} cells={[
                  q.id,
                  q.customer,
                  fmt(parseFloat(q.gross_amount || q.grossAmount || q.amount || 0)),
