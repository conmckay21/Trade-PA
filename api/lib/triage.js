...
- For ALL kinds where it'd help the founder: also produce draft_solution — plain-English direction on what to DO about the ticket. Tailor to kind:
    * bug — likely fix + where to look. Reference Trade PA's actual layout (monorepo, dual roots): /api/*.js at the repo ROOT for serverless functions (subfolders calls, cron, lib for shared helpers like resend.js/sentry.js/auth.js/triage.js, push, quickbooks, reminders, stripe with Connect + webhooks, xero), with serverless deps in ROOT package.json. /trade-pa/ holds the Vite + React app: trade-pa/src/App.jsx is the ~6k-line app shell; the rest is structured into trade-pa/src/views/ (26 page views like Dashboard, Jobs, Invoices, Schedule, Customers, Reports), components/ (27 shared UI bits including the voice panel and mic buttons), modals/, lib/ (23 utility modules), hooks/ (useWhisper, useReminders, useOnlineStatus), auth/, ai/AIAssistant.jsx, notifications/, theme/. Migrations: trade-pa/supabase/migrations/. Voice cascade: Grok STT/TTS → Deepgram → Whisper. Mobile: Capacitor v7 wrapping the Vite build (Android shipped, iOS pending). Give a likely root cause + a verification step. Not code — direction. Example: "Audio session likely times out when the screen sleeps. Check the voice pipeline initialisation in trade-pa/src/ai/AIAssistant.jsx or wherever the Grok STT recording starts, and verify navigator.wakeLock.request('screen') is held for the duration of recording on Android."
