...
async function chaseOverdueInvoices(conn, token) {
  // Get all invoices for this user from Supabase
  const res = await fetch(
    `${process.env.VITE_SUPABASE_URL}/rest/v1/invoices?user_id=eq.${conn.user_id}&status=eq.overdue&select=*`,
    { headers: { "apikey": process.env.SUPABASE_SERVICE_KEY, "Authorization": `Bearer ${process.env.SUPABASE_SERVICE_KEY}` } }
  );
  const invoices = await res.json();
  if (!invoices?.length) return 0;

  // Get customers for email lookup
  const custRes = await fetch(
    `${process.env.VITE_SUPABASE_URL}/rest/v1/customers?user_id=eq.${conn.user_id}&select=*`,
    { headers: { "apikey": process.env.SUPABASE_SERVICE_KEY, "Authorization": `Bearer ${process.env.SUPABASE_SERVICE_KEY}` } }
  );
  ...
  for (const inv of invoices) {
    ...
    // Only chase once per week — check last_chased field
    if (inv.last_chased) {
      const daysSinceChase = (Date.now() - new Date(inv.last_chased)) / 86400000;
      if (daysSinceChase < 7) continue;
    }
    ...
    const body = `<p>Dear ${inv.customer},</p>
<p>I hope you're well. I'm writing to follow up on invoice <strong>${inv.id}</strong> for <strong>£${inv.amount}</strong>, which is now overdue.</p>
<p>Could you please arrange payment at your earliest convenience? Payment can be made by bank transfer:</p>
<p>If you have already made payment, please disregard this message and accept my apologies for any inconvenience.</p>
<p>If you have any queries regarding this invoice, please don't hesitate to get in touch.</p>
<p>Many thanks</p>`;
    ...
    try {
      ...
      // Update last_chased timestamp
      await fetch(`${process.env.VITE_SUPABASE_URL}/rest/v1/invoices?id=eq.${inv.id}`, {
        method: "PATCH",
        headers: { "apikey": process.env.SUPABASE_SERVICE_KEY, "Authorization": `Bearer ${process.env.SUPABASE_SERVICE_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ last_chased: new Date().toISOString() }),
      });
      chased++;
    } catch (err) {
  ...
  try {
    ...
    for (const conn of connections) {
      try {
        ...
        // Chase any overdue invoices (max once per week per invoice)
        try {
          const chased = await chaseOverdueInvoices(conn, token);
          if (chased > 0) console.log(`Chased ${chased} overdue invoices for user ${conn.user_id}`);
        } catch (err) {
          console.error(`Chase overdue failed for ${conn.user_id}:`, err.message);
        }
      } catch (err) {
